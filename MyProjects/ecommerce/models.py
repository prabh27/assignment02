...
class Bridge(models.Model):
    bridge_id = models.IntegerField(primary_key=True)
...
class Products(models.Model):
    ...
    def get_category_id(self):
        if Bridge.objects.filter(product_id=self.product_id):
            obj = Bridge.objects.filter(product_id=self.product_id)
            return obj[0].category_id
        else:
            return None

    # def get_category_name(self):
    #     obj = Bridge.objects.get(category__category_name=self.name)
    #     return  obj.id
